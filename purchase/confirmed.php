...
include_once('../bootstrap.php');
  ...
  $id=$_POST['id'];
  $sql5 = "SELECT username,supid,pur_id,itemname,item_code,item_qty,cgst,sgst,igst,batch,mrp,total FROM cartpur WHERE username='$username'";
  ...
  if ($result5->num_rows > 0)
  {
    ...
    $mrp=$row['mrp'];
    $total=$row['total'];
    $stmt=$conn->prepare("INSERT INTO purchased(username,supid,pur_id,itemname,item_code,item_qty,cgst,sgst,igst,batch,mrp,total)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
    $stmt->bind_param("sssssidddsdd",$username,$supid,$purid,$itemname,$itemcode,$itemqty,$cgst,$sgst,$igst,$batch,$mrp,$total);
    $stmt->execute();
    //stock
    $SELECT = "SELECT itemcode From stock Where itemcode = ? ";
  ...
  $stmt=$conn->prepare("INSERT INTO purchase(id,pur_id,pur_date,sup_id,gtotal) VALUES (?,?,?,?,?)");
...
<body>
  ...
  <center><h3>SUCCESSFULLY PURCHASED<h3></center>
