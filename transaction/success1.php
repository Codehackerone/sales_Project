...
$sql = "SELECT cust_id,cust_name,cust_add,cust_phno,state,stateid,gstin FROM customer WHERE cust_name LIKE '%$supname%'";
...
<tbody>
  <tr id="table1"><td><input list="browsers" name="itemname1" class="input2" placeholder="ITEM NAME" id="itemname1" required onchange="showCustomer(this.value,1)"></td>
          <td><input type="text" required placeholder="ITEM CODE" class="input2" name="itemcode1" id="itemcode1" required readonly ></td>
                       <td><input type="number" readonly class="input2" id="stock1" placeholder="ITEM STOCK"></td>
          <td><input type="number" required placeholder="ITEM QUANTITY" class="input2" name="itemqty1"
             id="itemqty1" required onkeyup="myKeyfunc(this.value,1)"></td>
        ...
        <td><input type="decimal" required placeholder="MRP" class="input2" name="mrp1" id="mrp1" required readonly></td>
...
function showCustomer(str,counterval) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      var newcode5=document.getElementById("stocki").value;
      document.getElementById("stock"+counterval).value = newcode5;
      if(newcode5==0)
      {
        Swal.fire({
            position: 'top-end',
          icon: 'error',
          title: 'ITEM '+str+' HAS 0 QUANTITY. PURCHASE FIRST.',
          showConfirmButton: false,
          timer: 2000
            });
      }
      document.getElementById("itemqty"+counterval).max=newcode5;
    }
  };
...
function myKeyfunc(value,counterval2)
{
  ...
  var i=document.getElementById('stock'+counterval2).value;
  if(x > parseInt(i))
  {
    Swal.fire({
      position:'top-end',
      icon:'error',
      title:'YOU DONT HAVE ENOUGH QUANTITY IN STOCK.',
      showConfirmButton:false,
      timer:2000
    });
  }
...
$(document).ready(function(){
  $("#btn1").click(function(){
    ...
    $("#myTable tbody").append(
      ...
      ' <td><input type="number" readonly class="input2" id="stock'+srno+'" placeholder="ITEM STOCK"></td>'+
